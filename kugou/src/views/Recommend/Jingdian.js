...
import {
 ...
 class Jingdian extends React.Component{
	constructor(props) {
	   super(props);
	   this.state={
		 child:[],
					...
					<div>
						{
							...
							this.state.child.map((v,i)=>{
								...
								return(
								<Link  key={i} to={"/Xjingdian/"+v.sourceid}>
	...
	jiazai(fn){
		 this.axios.get("/kugou/q.k?op=query&cont=ninfo&node=44&pn="+fn+"&rn=15&fmt=json&src=mobile")
				.then(data=>{
					this.setState({
...
export default withRouter(Jingdian)