...
import {
 ...
 class Tianmi extends React.Component{
	constructor(props) {
	   super(props);
	   this.state={
		 child:[],
        ...
        return (
            <div>
				...
				<div className="heder-1">
					...
					<span className="context-1">甜蜜</span>
					...
					<div>
						{
							...
							this.state.child.map((v,i)=>{
								...
								return(
								<Link  key={i} to={"/Xiangqing/"+v.sourceid}>
	...
	jiazai(fn){
		 this.axios.get("/kugou/q.k?op=query&cont=ninfo&node=88114&pn="+fn+"&rn=15&fmt=json&src=mobile")
				.then(data=>{
					this.setState({
...
export default withRouter(Tianmi)